...
class Config extends Controller
{
    ...
    function validar_temp($token,$temp_I){
        ...
        if($dataTemp['modo_temp']!=$temp_I){
            if ($temp_I=="F" || $temp_I=="C") {
                $mensaje= "ok";
            }else{ $mensaje= "Wrong temperature format";}
    ...
    public function Gmt($token,$param){
        ...
        }else{$validacion= "Didn't enter GMT";}
    ...
    public function MyConfig($token,$param){
        ...
        if($validarUsuario){
            ...
            $data['Role'] =$rol;
            $validacion = "Successful application";
        }else{$validacion=$validarUsuario;}
        echo json_encode($this->respuesta($validacion,$data));  
    }
        ...
        if ($superUsuario['id']==1) {
            ...
            if($userM == "user" ){$validacion= "No user login for intervention";
                ...
                if($validarUsuario){
                    ...
                    if(strtolower($accion)== "disabled"||strtolower($accion)== "enabled"||strtolower($accion)== "resetpassword" ||strtolower($accion)== "gmt"||strtolower($accion)== "role"||strtolower($accion)== "temp"){
